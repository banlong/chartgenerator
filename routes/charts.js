...
router.post('/:chartId', function(req, res, next) {
    ...
    createChart(chartType, model, res);
...
//Using jsdom.env
function createChart(chartType, model, res){
    var data = model.data;
    ...
    jsdom.env(
        ...
        function (err, window) {
            var input = {
             ...
             switch (chartType){
                 case "bubble":
                     drawBubbleChart(input);
                     break;
                 case "line":
                     drawLineChart(input);
                     break;
                 case "gauge":
                     drawLiquidGauge(input);
                     break;
             }
        });
}
...
function drawLiquidGauge(input){
    ...
    for(var i = 0; i< scoreData.length; i++){
        //Gauge data
        var scoreNode = scoreData[i];
        var scoreId = scoreNode[0] + scoreNode[2];
        var scoreValue = scoreNode[1];
